...
async def main(clientMqtt, server_state):
    ...
    try:
        while clientMqtt !=0:
            z = loop.create_task(create_database(firstTime))
            if firstTime:
                dataBase, opcServers, firstTime = await z
                dataDict, client, server_state = await create_dataDict(dataBase, opcServers, server_state)
                ...
                if dataBase != 0:
                    server_state = True
                    dataDict, client, server_state = await asyncio.create_task(
                        create_dataDict(dataBase, opcServers, server_state))
                    ...
                    pass
            # firstData = dataDict
            x = loop.create_task(get_values(dataDict, client))
            y = loop.create_task(get_timesync(q))
            timeSync = await y
            dataDict = await x
            print(timeSync)
            value = []
        ...
        await main(clientMqtt, server_state)
